@extends('layouts.app')
@section('title', 'Sửa Nhu yếu phẩm - ĐẠI PHÚC')
...
<div class="dash-layout" x-data="{ sidebarOpen: true }">
  ...
  <main class="dash-main">
    @include('partials.dashboard-header', ['pageTitle' => '✏️ Chỉnh sửa Nhu yếu phẩm'])
    <div style="padding:1.5rem">
      ...
      <div style="font-size:.83rem;color:#9ca3af;margin-bottom:1.25rem">
        ...
        &rsaquo; Sửa: <strong style="color:#1e293b">{{ $supply->name }}</strong>
      </div>
      ...
      <div class="chart-container" style="max-width:680px">
        ...
        <form action="{{ route('admin.supplies.update', $supply) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div style="margin-bottom:1.1rem">
            ...
            <select name="category_id"
              ...
              @foreach($categories as $cat)
                <option value="{{ $cat->id }}"
                  {{ old('category_id', $supply->category_id) == $cat->id ? 'selected' : '' }}>
                  {{ $cat->name }}
                </option>
              @endforeach
            </select>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.1rem">
            ...
            <div>
              ...
              <input type="number" name="min_stock_alert"
                     value="{{ old('min_stock_alert', $supply->min_stock_alert) }}" min="0"
