@extends('layouts.app')
@section('title', 'Sửa Kho hàng - ĐẠI PHÚC')
...
<div class="dash-layout" x-data="{ sidebarOpen: true }">
  ...
  <main class="dash-main">
    @include('partials.dashboard-header', ['pageTitle' => '✏️ Sửa Kho hàng'])
    <div style="padding:1.5rem">
      ...
      <div style="font-size:.83rem;color:#9ca3af;margin-bottom:1.25rem">
        ...
        &rsaquo; {{ $warehouse->name }}
      </div>

      {{-- FORM CARD --}}
      <div class="chart-container" style="max-width:760px">
        <h3 style="margin-bottom:1.5rem">🏭 Cập nhật thông tin kho</h3>
        ...
        <form action="{{ route('admin.warehouses.update', $warehouse) }}" method="POST">
          ...
          <div style="margin-bottom:1.1rem">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $warehouse->name) }}"
                   placeholder="Ví dụ: Kho Bình Thạnh..."
          ...
          <div style="margin-bottom:1.1rem">
            ...
            <textarea name="address" id="address" rows="2"
                      ...
                      style="width:100%;padding:.6rem .8rem;border:1px solid {{ $errors->has('address') ? '#ef4444' : '#d1d5db' }};border-radius:8px;font-size:.9rem;outline:none;box-sizing:border-box;resize:vertical">{{ old('address', $warehouse->address) }}</textarea>
            @error('address')
              <p style="color:#ef4444;font-size:.8rem;margin-top:.3rem">{{ $message }}</p>
            @enderror
          </div>
...
(function () {
  /* ========== KHỞI TẠO BẢN ĐỒ ========== */
  const DEFAULT_LAT = 10.7769, DEFAULT_LNG = 106.7009;
  ...
  const initZoom = document.getElementById('lat').value ? 15 : 13;
  ...
  function pickSuggestion(idx) {
    ...
    placeMarker(parseFloat(item.lat), parseFloat(item.lon), 16);
